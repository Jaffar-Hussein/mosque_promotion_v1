...
function About() {
	return (
		<section id="about" className="bg-white dark:bg-accentBase-dark py-20 px-4">
			<div className="max-w-6xl mx-auto grid md:grid-cols-2 gap-16 items-center">

				{/* Image */}
				<div className="flex justify-center relative">
					<div aria-hidden="true" className="absolute w-72 h-72 rounded-full bg-accentBgActive dark:bg-accentBgActive-dark opacity-50 blur-3xl top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2" />
					<img
						src={quran}
						alt="Open Quran"
				...
				<div className="flex flex-col gap-6">
					...
					{/* Stats */}
					<div className="grid grid-cols-3 gap-4 py-6 border-y border-accentLine dark:border-accentLine-dark">
						{stats.map(({ value, label }) => (
							<div key={label} className="text-center">
								<p className="font-heading text-3xl font-bold text-accentTextContrast dark:text-accentTextContrast-dark">
									{value}
								</p>
					...
					<a
						href="#courses"
						className="inline-flex items-center gap-2 self-start font-semibold text-accentSolid dark:text-accentText-dark hover:text-accentSolidHover dark:hover:text-accentTextContrast-dark transition-colors duration-200 group"
					>
						Browse our courses
